...
This spec landed in #1459 (`a720b9d`, 2026-09-02) with this file still an unfilled template. It was filled on 2026-09-23 after a retroactive review, run at the landing commit, failed it (round 1, below). Round 1's fixes merged in #1655; round 2 failed them on the findings below, and round 3's fixes are on `fix/sec001-round3`. Every check in `features.json` is a runnable command, and each one exits 0 on this branch.
...
| AC5 | `TestAssertSafeChildCommand`: 40 rows, 32 refused and 8 allowed | `cli/internal/cmd/secrets_test.go` |
...
- `go test ./... -count=1`: 25 packages ok, 0 failed (round 3).
...
- Every `features.json` verifier exits 0 (round 3). Two negative controls exit 1: a `-run` pattern that matches no test, and a deny rule that is not in the file.
