...
# Tasks - TEST-001-bats-coverage-gaps

> TDD order. One task = one focused commit. Tick as you go. Reorder freely while spec is in `draft` state; freeze once you start `implementing`.

## Setup

- [ ] Branch created from main: `feat/TEST-001-bats-coverage-gaps`
...
Minimal `features.json` skeleton (drop into `<repo>/specs/TEST-001-bats-coverage-gaps/features.json`):

```json
[
  {
    "id": "TEST-001-bats-coverage-gaps-f1",
