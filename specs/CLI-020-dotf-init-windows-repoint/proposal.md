...
- **Parity gate (BLOCKING — verify before any deletion).** `dotf init` on Windows must create the Knowledge-Vault entry identically to `init-project.ps1`. `cli/internal/vault/project.go:62` builds `VaultPath/10_projects/<repo>`; confirm `VaultPath` resolves on Windows (`dotf env path VAULT_PATH` / `machine.json`, ADR-025 — `dotf env generate` already runs at `setup-windows.ps1:1556`) and that the entry path + templates match the `.ps1` output. If they diverge, the deletion is blocked until `dotf init` is fixed.
