...
- `dotf harness bind --harness agy`, rehearsed first on copies of the three files: Orca's `orca-status` hook byte-identical, ours added (`PreToolUse`, matcher `*`), only our `BeforeTool` handler retired from `~/.gemini/settings.json`, modes kept. The live files equal the rehearsal, and a second run reports nothing to do.
