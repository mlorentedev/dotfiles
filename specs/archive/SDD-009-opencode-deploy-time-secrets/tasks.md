...
# Tasks - SDD-009-opencode-deploy-time-secrets

> TDD order. One task = one focused commit. Tick as you go. Reorder freely while spec is in `draft` state; freeze once you start `implementing`.

## Setup

- [ ] Branch created from main: `feat/SDD-009-opencode-deploy-time-secrets`
...
Minimal `features.json` skeleton (drop into `<repo>/specs/SDD-009-opencode-deploy-time-secrets/features.json`):
