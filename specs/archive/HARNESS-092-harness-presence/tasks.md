...
- [x] [P] [AC3] `compile-harness.sh`: `deploy_agent_presence` delegates, `build_agent_presence` wraps `--render`, `inject_agent_presence` deleted; absent/old dotf → loud WARN `presence NOT deployed`, the deploy continues; a verb that runs and fails does fail `--deploy`
