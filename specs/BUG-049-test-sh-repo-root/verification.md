...
Differential verification: the same suite, both platforms, before and after. There is
no unit-test surface here — `scripts/test.sh` *is* the suite — so the proof is the
comparison, not a new assertion file.
...
- [x] **AC1 — Tests the committed tree** -> header changed from `Using custom DOTFILES_DIR: C:\Users\mlorente\.dotfiles` + `Testing from: C:\Users\mlorente\.dotfiles` to `Testing from: /c/Users/mlorente/Projects/Workspace/dotfiles-wt-testsh`, with `$DOTFILES_DIR` still exported in the shell.
