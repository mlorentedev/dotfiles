...
- **OPEN for Step 4f — `--work-sdk`.** init-project.sh's vault-only mode (`--work-sdk <family> <component>` → a `50_work/45-development/…` vault entry, no repo) is *not* ported by the orchestrator. It is referenced by `claude-session-start.{sh,ps1}` too. Deleting init-project.sh fully requires deciding: port it to `dotf vault` / drop it / keep init-project.sh alive solely for it. Tracked as the gating decision for the 4f deletion.
