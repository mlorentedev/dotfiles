...
- **`--work-sdk` — DECIDED for Step 4f: extract.** init-project.sh's vault-only mode (`--work-sdk <family> <component>` → a `50_work/45-development/…` vault entry, no repo; also referenced by `claude-session-start.{sh,ps1}`) is *not* ported by the `dotf init` orchestrator — it is vault work, not repo-scaffolding. Resolution (user picked "cleanest + most complete"): **extract it to a standalone transitional `init-work-sdk.sh`** so init-project.sh's repo-scaffolding can be deleted, and **file a ticket to port `--work-sdk` to a future `dotf vault`** (its proper home per ADR-022; vault is the next flagship in the port queue). Porting it *into* CLI-014 (option B) was rejected as scope-bleeding the init flagship into the vault flagship; dropping it (option C) was rejected because the capability is still referenced/used.
