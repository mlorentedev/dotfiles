...
- [x] AC2 (ZOHO_RECOVERY_CODE unaffected) -> live: `dotf secrets migrate ZOHO_RECOVERY_CODE --dry-run` fails with the same pre-existing `bw get item "zoho": More than one result was found` error (#962), unchanged by this diff
- [x] AC3 (byte-exact end-to-end test) -> test `TestSecretsMigrate_FileSecret_ByteExact` (multi-line fixture with NO trailing newline; asserts the bw-written value is exactly the fixture, neither trimmed nor appended)
