...
> Status: **implementing** — config/workflow/retirement landed and statically verified; the end-to-end release behaviour (AC4) is observable only after merge + the `RELEASE_TOKEN` secret exists.
...
- [~] AC4 (tag triggers goreleaser) -> workflow wires `secrets.RELEASE_TOKEN` (static check passes); **end-to-end pending** the secret + first real release
...
- **R1 token:** the default `GITHUB_TOKEN` will not trigger `cli.yml` on the tag it pushes (GitHub recursion guard) -> goreleaser would silently never run. Resolved with a dedicated PAT `RELEASE_TOKEN` (on-brand with OPS-007/#321). Hard dependency on the user provisioning the secret.
