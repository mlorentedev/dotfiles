...
- [x] Open question R1 (token) resolved as a documented dependency: a `RELEASE_PLEASE_TOKEN` PAT secret (the default `GITHUB_TOKEN` would not trigger goreleaser)
...
- [x] `.github/workflows/release-please.yml` on `push: main`, running `googleapis/release-please-action@v4` with `secrets.RELEASE_PLEASE_TOKEN` (R1)
...
- [ ] **USER:** create a fine-grained PAT (`contents: write` + `pull-requests: write`) and add it as the repo Actions secret `RELEASE_PLEASE_TOKEN` (ties to OPS-007/#321). Until then the release PR + tag work but goreleaser does not auto-fire.
