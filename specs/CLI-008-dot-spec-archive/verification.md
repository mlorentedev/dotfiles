...
- **No feature-id grammar validation in `archive`** (unlike `init`). `archive-spec.sh` gates only on directory existence; validating the grammar would block archiving a legitimately-named-but-edge-case spec. The dir-exists check is the only gate — faithful to the shell.
