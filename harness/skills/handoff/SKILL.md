...
- **Content:** frontmatter (`id` — `session-` + the filename slug, e.g. `session-2026-08-07-ts-bridge-claude`, the convention in 139 of 178 existing notes; `type: session`; `status: active`; `tags` — at minimum `[session, <project>]`, required by the Frontmatter Law; `date`; `agent`; `project`; `session_id` if the runtime exposes one). **Emit `id`/`type`/`status` as the FIRST three keys**, ahead of `date`/`agent`/`project`: some global `vault_health` sweeps mis-report verifiably compliant files as missing these keys (mechanism unconfirmed — a `project=`-scoped run of the same check reports the same files clean; see the open hive ticket). Leading with the three validated keys avoids every observed case + the same four handoff fields as step 1, but here you MAY be fuller than the ~8-line cap — this is the journal, not the snapshot. **`id`, `type` and `status` are the three keys `vault_health` validates** — a note that omits them lands as a frontmatter error, which is how 130 of 196 session files came to be non-compliant before this was specified.
