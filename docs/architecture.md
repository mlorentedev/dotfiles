...
| `forge/` | GitHub-side state declared in git: `branch-protection.json` and its schema (GUARD-017), read by `dotf forge protection check` and the `dotf doctor` branch-protection section |
