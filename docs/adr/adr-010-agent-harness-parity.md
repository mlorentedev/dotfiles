...
| **Hooks (SessionStart, PreToolUse, etc.)** | ✅ `settings.json` hooks | ⚠ `opencode plugin` (npm-distributed JS, different paradigm) | ✅ Gemini CLI: `~/.gemini/settings.json` hooks; agy reads `~/.gemini/config/hooks.json` instead (amended 2026-09-24, ADR-027) (BeforeAgent/BeforeTool; measured 2026-08-26, `harness/manifest.json` `bind_comment`) | ✅ `~/.copilot/hooks/*.json` (SessionStart/PreToolUse, deny by exit 2 or `permissionDecision`; measured 2026-08-29 on CLI 1.0.81 — ADR-027 amendment) | ⚠ unknown | ⚠ unknown |
