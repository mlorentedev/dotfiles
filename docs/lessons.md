...
**Context**: Adding the archive-on-merge half of the SDD Discipline Gate (#670) to `check-spec-gate.sh`, which runs under `set -euo pipefail`. The suite was written first and deliberately covered both directions: nine tests asserting the gate *fires* on a violation, ten asserting it stays silent on `Refs #N`, on a prose mention, on a cross-repo reference, on an empty PR body, on a spec with no `issue:` frontmatter.
