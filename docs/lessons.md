...
### [2026-08-15] A gate that scans a directory will eventually scan its own evidence
...
### [2026-08-16] Every layer reported a health none of them had established
...
### [2026-08-16] Under squash-merge, `git branch --merged` says no about every branch that landed
...
**Solution**: ask the forge, not the graph. `gh pr list --state all --head <branch> --json state` reports what actually happened to the change, and it stays correct under squash, rebase and merge-commit alike. Verified each of the four against its PR number before deleting, and left the parallel session's branch alone because it had no merged PR — the opposite of what git advised in both cases.
