...
|------------|---------------|---------|
| **Vault location** | `$VAULT_PATH` (default `~/Projects/knowledge`; per-machine override in `~/.config/dotfiles/machine.json`, ADR-025) | Setup scripts, session hooks, crystallize scripts |
...
If the vault moves to a different path, set its override in `~/.config/dotfiles/machine.json` and run `dotf env generate` (ADR-025) — no hand-editing of setup scripts or hooks. Projects must still follow the `~/Projects/<name>` naming convention.
...
| Vault path moved | Set `VAULT_PATH` (and `HIVE_VAULT_PATH`) in `~/.config/dotfiles/machine.json`, run `dotf env generate` (ADR-025) |
