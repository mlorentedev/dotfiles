...
// isHostProcessInside answers `true` unconditionally off Linux, because there is
// no implementation here and the caller deletes the worktree on a `false`.
//
...
func isHostProcessInside(_ string) bool {
	return true
}
