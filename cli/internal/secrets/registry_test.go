...
// check alone would let an app-plane secret declare infra (both strings are
...
func TestParseRegistry_BwFolder_ValidatedWhileDormant(t *testing.T) {
	const yml = "version: 1\nsecrets:\n" +
		"  - {id: a, plane: app, backend: age, age: f, bw: {item: it, field: password, folder: typo}, expose: {env: A}}\n"
	if _, err := ParseRegistry([]byte(yml)); err == nil {
