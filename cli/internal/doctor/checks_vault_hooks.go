...
func checkVaultHooks(sys *System, rep *Report, fix bool) {
	...
	// The gate is "active" only when BOTH stages are installed by pre-commit:
...
// vaultHookInstalled reports whether .git/hooks/<hookType> exists AND was written
// by pre-commit (it stamps every generated hook with this marker). The marker
// check rejects a foreign or hand-rolled hook squatting the path — only a
// pre-commit-managed hook actually runs the vault's gate.
func vaultHookInstalled(vault, hookType string) bool {
	return fileContains(filepath.Join(vault, ".git", "hooks", hookType), "generated by pre-commit")
}
