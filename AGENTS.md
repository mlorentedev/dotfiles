...
5. **Consult patterns before architectural decisions.** 37 universal patterns in `00_meta/patterns/`. Query via Hive MCP (when available) or read from `$VAULT_PATH/00_meta/patterns/<name>.md` (`$VAULT_PATH` defaults to `~/Projects/knowledge`; per-machine override in `~/.config/dotfiles/machine.json`, ADR-025).
...
**CORE PRINCIPLE:** Code lives in Git. **Knowledge placement is by layer — decide-vs-operate** (Standing Order #2): a repo's build/operate docs — ADRs (`docs/adr/`), runbooks, troubleshooting — live in that repo's `docs/`; the vault (`$VAULT_PATH` — default `~/Projects/knowledge/` Linux/macOS, `%USERPROFILE%\Projects\knowledge\` Windows; per-machine override in `~/.config/dotfiles/machine.json`, ADR-025) holds the cross-project brain, AI memory, and personal/methodology lessons. Patterns: `pattern-knowledge-placement`, `pattern-platform-governance`.
...
This repo follows the **Spec-Driven Development per feature** pattern. Canonical workflow definition at `$VAULT_PATH/00_meta/skills/spec/SKILL.md` (`$VAULT_PATH` defaults to `~/Projects/knowledge`; per-machine override in `~/.config/dotfiles/machine.json`, ADR-025).
